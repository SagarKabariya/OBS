...
<?php
		...
		include("head.php");
		include("conn.php");
		$getmain="select * from main_book_cat";
		...
		if(isset($_POST['Submit']))
		{
			...
			$sdes=$_POST['sdesc'];
			$ldes=$_POST['ldesc'];
			$maincid=$_POST['maincat'];
			$subcid=$_POST['subcat'];
			$autid=$_POST['auth'];
			$pubid=$_POST['publi'];
			$reld=$_POST['txtCalendar'];
			$edi=$_POST['edition'];
			
			$getmaincid="SELECT `main_book_id` FROM `main_book_cat` WHERE `main_cat_name` like '$maincid'";
			...
			$qrygetmainid=mysql_query($getmaincid);
			$aa=mysql_fetch_row($qrygetmainid);
			$finmainid=$aa[0];
			...
			$allowedExts = array("odt", "pdf", "txt","doc");
				...
				&& in_array($extensionimg, $allowedExtsimg))
				  ...
				  $finalqry="INSERT INTO `user_share`(`user_id`, `main_book_cat_id`, `sub_book_cat_id`, `book_title`, `sdesc`, `ldesc`, `publisher_id`, `author_id`, `edition`, `book_path`, `image_path`,`rel_date`) VALUES ('$userid','$finmainid','$finsubid','$bookname','$sdes','$ldes','$finpubid','$finauthid','$edi','$finalpath','$finalimgpath','$reld')";
	  ...
	  <form method="post" enctype='multipart/form-data'>
		...
		<div class="content">
		  <table class="form">
            ...
            <tr>
          <td><span class="required">*</span> Main Category:</td>
          <td>
          <select id="Maincat" name="maincat" calss="dropdown-menu">
          <option value="">-----Select Main Category-----</option>
          <?php
		  			$resmain=mysql_query($getmain);
					while($row=mysql_fetch_array($resmain))
					{
